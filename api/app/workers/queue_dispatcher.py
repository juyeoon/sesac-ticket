...
[담당] A
[역할] 주기적으로 대기열 앞쪽 N명을 ZPOPMIN → entryTicket 발급 → READY 전환.

[구현할 것]
- dispatch_once(schedule_id) -> int (발급된 인원 수)
...
def dispatch_once(schedule_id: int) -> int:
    settings = get_settings()
    client = get_master_client()
    popped = client.zpopmin(queue_key(schedule_id), settings.queue_dispatch_batch_size)

    for member_id_raw, _score in popped:
        member_id = int(member_id_raw)
        ticket_id = uuid.uuid4().hex
        client.set(entry_ticket_key(ticket_id), str(member_id), ex=_ENTRY_TICKET_TTL_SEC)
        client.set(queue_ready_key(schedule_id, member_id), ticket_id, ex=_ENTRY_TICKET_TTL_SEC)

    return len(popped)


def _dispatch_all_schedules() -> None:
    # TODO: B의 performance/schedule 도메인이 준비되면 활성 회차 목록을 조회해
    # 각 schedule_id에 대해 dispatch_once를 호출하도록 교체한다.
