...
관리자 계정(`src/mocks/handlers/admin.ts`에 하드코딩, DB 없이 고정값 비교):
...
**Phase 0~5 전체 화면 구현 완료.** 다음 단계는 화면 추가가 아니라 [`frontend-handoff.md`](../../docs/frontend-handoff.md) 4번에 정리된 대로 Swagger 기준 실 API 연동.
...
3. 좌석 배치도 범례 4가지(예매가능/선택중/선점중/예매완료) 색이 다르게 보이는지
...
1. 비로그인 상태에서 `/mypage` 직접 접속 → "로그인이 필요해요" 안내 화면(홈으로 가기 버튼)이 뜨는지
2. 로그인 후 헤더 아바타 메뉴 → "마이페이지" → 내 정보(이메일/닉네임/성별/나이대) 조회되는지
3. "정보 수정" → 닉네임 변경 → "인증번호 발송" → 콘솔에서 `[mock] {이메일} 인증코드: 123456` 확인 후 입력 → 저장 → `/mypage`로 돌아와 바뀐 닉네임이 보이는지
4. 인증번호를 안 넣거나 틀리게 넣고 저장 시 "인증코드가 일치하지 않습니다" 에러가 뜨는지
5. "내 예매 목록" 탭 → 무통장입금 예매를 만든 적이 있다면 공연명/날짜/상태 배지가 보이고, 클릭 시 예매 확인 페이지(`/reservations/bank-transfer/:id`)로 이동하는지
6. "관심 공연" 탭 → 공연 상세에서 하트를 눌러 등록한 공연이 카드로 보이는지, 하트 아이콘 클릭 시 목록에서 바로 빠지는지

### 고객센터 (`/support`, `/support/:postId`)
1. 카테고리 칩(전체/공지/이용안내/자주묻는질문) 클릭 시 목록이 필터링되는지, URL에 `?category=`가 붙는지
2. 게시글이 6개 넘는 카테고리(전체, 자주묻는질문)에서 하단 페이지네이션이 보이고 2페이지로 넘어가는지
3. 게시글 클릭 → 상세 페이지에서 제목/카테고리/작성일/본문 확인, "목록으로" 클릭 시 되돌아가는지
4. 로그인 여부와 무관하게(비로그인 상태에서도) 목록·상세 둘 다 조회되는지

### 관리자 로그인 (`/admin/login`, `/admin`)
1. `admin` / `admin1234`로 로그인 → `/admin`으로 이동, "admin님 환영합니다" 확인
2. 잘못된 계정으로 로그인 시도 → "관리자 ID 또는 비밀번호가 올바르지 않습니다" 에러
3. 로그인 상태에서 새로고침(F5) → 일반 회원과 동일하게 accessToken이 메모리에만 있어서 로그아웃되는지, `/admin` 직접 접속 시 "로그인이 필요해요" 안내(관리자 로그인으로 이동 버튼)가 뜨는지
4. "로그아웃" 클릭 → `/admin/login`으로 이동
5. 이 화면은 일반 사용자 헤더/푸터(`RootLayout`)를 안 쓰는지 확인(디자인 시스템 컬러는 동일하게 적용됨)
...
| entryTicket 캐시 | 스케줄별 세션스토리지 4분 캐시 | 같은 세션 재방문 시 대기열 재진입 방지. 처음엔 10분일 거라 추측해 9분으로 잡았는데, 실제 TTL이 5분(`_ENTRY_TICKET_TTL_SEC=300`, 하드코딩값 — 확인됨)이라 만료 직전 재사용을 막도록 4분으로 정정 |
| 좌석 최대 선택 수 | 4석 | 스펙에 명시 없음 — 일반적인 티켓팅 관례로 추가 |
| 좌석 상태 시드 | seatId 7의 배수=RESERVED, 11의 배수=HELD, 나머지=AVAILABLE | 데모에서 3가지 상태를 항상 골고루 보여주려고. 원래 SOLD로 시드했었는데 실제 백엔드엔 그 상태가 없고 RESERVED만 있다고 확인돼 정정 |
...
| 마이페이지 나이대 선택지 | "10대/20대/30대/40대/50대 이상" 5개 고정 | 스펙엔 `ageRange`가 문자열 optional일 뿐 값 목록이 없음 — 실제 연동 시 백엔드가 쓰는 값과 다르면 옵션 값만 교체 |
| 내 정보 수정(PATCH `/users/me`) | mock도 회원가입과 동일하게 `verificationCode`를 발급/검증하도록 구현 | 실제 스펙이 이 필드를 필수로 요구한다고 확인됨(`frontend-handoff.md`) — mock이 검증을 안 하면 실 서버 연동 시 폼이 갑자기 400을 받아 당황하게 되므로 미리 맞춰둠 |
| 고객센터 카테고리 목록 | "공지/이용안내/자주묻는질문" 3개 고정(`mocks/data/support.ts`의 `SUPPORT_CATEGORIES`) | 스펙엔 `category`가 자유 문자열일 뿐 값 목록 자체가 없음(실제 서버도 admin이 게시글 작성 시 입력하는 임의 문자열) — 실 연동 시 실제 운영 중인 카테고리 값으로 교체 |
| 고객센터 페이지 크기 | 6개(백엔드 기본값은 20) | mock 시드가 12개뿐이라 페이지네이션 동작을 눈으로 확인하려고 작게 잡음 — 실 연동 시 원하는 값으로 조정 |
| 관리자 계정 | `admin`/`admin1234` 하드코딩(`mocks/handlers/admin.ts`), 회원 mock DB(`db.ts`)와 분리 | 실 백엔드도 관리자를 별도 `admin` 테이블 + 별도 refresh 쿠키(`adminRefreshToken`)로 완전히 분리해서 관리하길래 mock도 그 구조를 따라감 |
| 관리자 세션 | Refresh Token 미구현, accessToken 메모리만 보관(새로고침하면 로그아웃) | 일반 회원과 동일한 의도적 단순화 — 실제 `/admin/auth/refresh`(쿠키 기반)는 확인됐지만 Phase 5 범위는 로그인 화면 자체라 다음 API 연동 단계로 미룸 |
| 관리자 홈(`/admin`) | 로그인 성공 후 보여줄 실제 대시보드가 없어서 환영 문구 + 로그아웃 버튼만 있는 placeholder | 백엔드 admin 도메인엔 로그인/재발급 API만 있고 관리 기능(공연 등록 등) API가 아직 없음 — 나올 때 채울 것 |
